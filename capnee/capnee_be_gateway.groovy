                ...
                script {
                    ...
                    if (Ambiente == 'PROD') {
                        ...
                        withCredentials([usernamePassword(credentialsId: 'clave', passwordVariable: 'PASSWORD', usernameVariable: 'USERNAME')]) {
                            def result = sh(script: "sshpass -p '${PASSWORD}' ssh -o StrictHostKeyChecking=no -p 5948 ${USERNAME}@200.58.106.151 'bash /root/capnee/deploy_capnee_be_gateway.sh ${Branch}'", returnStatus: true)
                            if (result != 0) {
                        ...
                        def result = sh(script: "sh /home/administrador/projects/capnee/deploy_capnee_be_gateway.sh ${Branch}", returnStatus: true)
                        if (result != 0) {
